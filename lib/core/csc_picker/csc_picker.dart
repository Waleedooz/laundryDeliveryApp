...
class CSCPickerState extends State<CSCPicker> {
  ...
  void initState() {
    ...
    //   _onSelectedCountry(widget.selectedCountry!);
    // }
  }
  ...
  void _onDefaultSelectionSet(String country, String state, String city) {
    print("ondefault===true${widget.selectedCountry}");
    if (!mounted) return;
    setState(() {
      // if (widget.flagState == CountryFlag.SHOW_IN_DROP_DOWN_ONLY) {
      //   try {
      //     widget.onCountryChanged!(country.substring(6).trim());
      //   } catch (e) {}
      // } else {
            ...
            : Column(
                children: [
                 ...
                 SizedBox(height: getVerticalSize(16),),
                  widget.showStates ?   SizedBox(width: getHorizontalSize(16)) : Container(),
                  widget.showStates
                      ? stateDropdown()
                      : Container(),
                  SizedBox(height: getVerticalSize(16),),
  ...
  Widget countryDropdown() {
    print(
        "countru===${_country.isEmpty || widget.disableCountry ? true : false}");
    return SizedBox(
      height: getSize(52),
